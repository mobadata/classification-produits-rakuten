...
import utils_efficientnet_b3 as utils_efficientnet_b3
...
def randomInput():
    ...
    st.session_state['class_input'] = prdtypecode_labels[product.iloc[0]['prdtypecode']]
...
def renderDemonstration():
    ...
    with st.container():
        ...
        with col1:
            ...
            st.write(f"Poids CamemBERT: {text_weight:.2f} - Poids efficientnet_b3: {1 - text_weight:.2f}")
        ...
        with col2:
            if submitted:
                ...
                with st.spinner('Classification en cours, veuillez patienter....'):
                    ...
                    if uploaded_file is not None:
                        uploaded_file.load()
                        img_resized = uploaded_file.resize((300, 300))  # Fixed: EfficientNetB3 expects 300x300
                        img_array = np.asarray(img_resized)
                        image = img_array.reshape((300, 300, 3))  # Fixed: EfficientNetB3 expects 300x300
                        image_predictions = utils_efficientnet_b3.predict(image,utils_camembert.classes_order)
                ...
                try: 
                    ...
                    class_labels = [prdtypecode_labels[class_label] \
...
def format_model_prediction_class_detail(model_predictions,image_or_text,fusion_prediction_index=None):
    ...
    model_prediction = prdtypecode_labels[
